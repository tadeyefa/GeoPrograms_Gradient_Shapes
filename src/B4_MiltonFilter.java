public class B4_MiltonFilter extends World
{
   ...
   public void go()
   {
      plane.loadBackGround("panthers.jpeg");          //*** new method to load a diffrent JPG
      System.out.println("This will be printed to the  window. ");
      plane.teleport(0,0);
      plane.showBackGround();                    
      System.out.println(plane.howMuchRed());         
   
      for(int row=300; row<400; row++)    //*** this is using new getBackground
      {
         for(int col=440; col<750; col++)    //*** this is using new getBackground
         {
            ...
            if((red>0 && green>50 && blue>80) && (red<100 && green<175 && blue<250))
            {
               plane.setPixelColor(155,25,18);
            }
